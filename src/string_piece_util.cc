...
vector<std::string_view> SplitStringView(std::string_view input, char sep) {
  ...
  for (;;) {
    std::string_view::size_type next_pos = input.find(sep, pos);
...
string JoinStringView(const vector<std::string_view>& list, char sep) {
  if (list.empty()) {
    return "";
  }
